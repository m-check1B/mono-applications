...
const loadMessages = async () => {
  try {
    const raw = await fs__default.readFile(BOARD_PATH, "utf-8");
...
const toPost = (message) => {
  ...
  return {
    ...
    created_at: message.time || (/* @__PURE__ */ new Date()).toISOString(),
    tags: topic ? [topic] : []
  };
};
const GET = async ({ params }) => {
  try {
    ...
    const posts = messages.filter((message) => (message.topic || "general") === boardId).sort((a, b) => String(b.time || "").localeCompare(String(a.time || ""))).map(toPost);
    return json({ posts });
  } catch (e) {
