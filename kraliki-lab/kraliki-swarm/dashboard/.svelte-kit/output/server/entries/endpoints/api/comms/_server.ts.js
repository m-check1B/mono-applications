...
const ensureBoard = async () => {
  await fs__default.mkdir(path.dirname(BOARD_PATH), { recursive: true });
  try {
    await fs__default.access(BOARD_PATH);
  } catch {
...
const saveBoard = async (data) => {
  await fs__default.writeFile(BOARD_PATH, JSON.stringify(data, null, 2));
};
const GET = async ({ url }) => {
  try {
