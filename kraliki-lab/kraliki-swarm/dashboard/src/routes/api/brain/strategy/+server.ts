...
import { getScopePath } from '$lib/server/scopes';
...
/**
 * Parse markdown file to extract title, status, and structure
 */
function parseMarkdown(content: string, filename: string): Partial<StrategyDoc> {
	...
	// Extract phases (H3 sections starting with "Phase" or "Step")
 ...
 */
async function scanDirectory(dirPath: string, type: 'decision' | 'plan' | 'roadmap'): Promise<StrategyDoc[]> {
...
/**
 * POST: Send strategy to Focus as a project
 */
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		// Create project in Focus by Kraliki
		...
		// Create tasks from parsed phases/tasks
		const createdTasks = [];
		const allTasks = [
			...(parsed.tasks || []),
