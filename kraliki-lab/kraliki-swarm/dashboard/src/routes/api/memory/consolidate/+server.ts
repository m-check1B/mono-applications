...
async function getConsolidationPlan(): Promise<ConsolidationPlan> {
	...
	const consolidations: Record<string, { sources: number; memories: number }> = {};
	...
	for (const file of files) {
		...
		if (!match) continue; // Not an ephemeral file
		...
		// Count lines (memories) in this file
		try {
			const filePath = `${memoriesPath}/${file}`;
			const { stdout } = await execAsync(`wc -l < "${filePath}"`, { timeout: 5000 });
	...
	return {
		...
		consolidations: Object.entries(consolidations)
			.map(([target, data]) => ({
				target: `${target}.jsonl`,
...
async function executeConsolidation(): Promise<ConsolidationResult> {
	const kralikiPath = await getScopePath('kraliki');
	if (!kralikiPath) {
...
// GET: Get consolidation plan (dry run)
