...
async function getKnownAgents(): Promise<string[]> {
	try {
		...
		return files
			...
			.map(f => f.replace('.md', ''))
			.filter(f => !f.includes('_prompt'));
	} catch {
		return [];
	}
}

async function getMemoriesFromFiles(): Promise<{ entries: MemoryEntry[], stats: MemoryStats }> {
	const entries: MemoryEntry[] = [];
	const byAgent: Record<string, { stores: number; retrieves: number }> = {};
				...
				for (const line of lines) {
					try {
						...
						entries.push({
							...
							timestamp: mem.metadata?.time || new Date().toISOString(),
							size: line.length,
							text: mem.text
						});
					} catch {
...
export const GET: RequestHandler = async () => {
	try {
		...
		// Calculate inactive agents (known agents with no memory activity)
