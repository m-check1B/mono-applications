...
// EspoCRM credentials from /github/secrets/espocrm.env
...
async function fetchEspo(endpoint: string): Promise<any> {
	try {
...
async function getStats(): Promise<CRMStats> {
...
async function getOpenOpportunities(): Promise<Opportunity[]> {
...
export const GET: RequestHandler = async () => {
	try {
		// Check if EspoCRM is running with authentication
		...
		if (!isOnline) {
			// Check if it's an auth issue vs server down
