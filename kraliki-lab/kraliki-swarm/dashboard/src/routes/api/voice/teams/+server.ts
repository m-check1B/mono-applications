...
/**
 * Voice by Kraliki teams endpoint
 * GET /api/voice/teams - List teams
 * POST /api/voice/teams - Create team
 */
export const GET: RequestHandler = async ({ cookies }) => {
  const headers = getKralikiHeaders(cookies);

  try {
    const response = await fetch(`${VOICE_URL}/api/v1/teams`, {
      headers,
      signal: AbortSignal.timeout(10000)
    });

    if (!response.ok) {
      return json({ error: 'Voice by Kraliki teams API error', status: response.status }, { status: response.status });
    }

    const data = await response.json();
    return json(data);
  } catch {
    return json({ error: 'Voice by Kraliki teams service unavailable' }, { status: 503 });
  }
};
