<script lang="ts">
	...
	let sessions = $derived(Object.values($chatStore.sessions));
	...
	async function loadSessions() {
		try {
			...
			const mappedSessions: ChatSession[] = response.sessions.map((session) => {
				...
				return {
					...
					createdAt: session.created_at ? new Date(session.created_at) : timestamp,
			...
			>
				...
				<div class="flex-1 min-w-0">
					<div class="flex items-center justify-between gap-2">
						...
						<span class="text-xs text-text-muted">{formatTime(session.lastActivity)}</span>
					</div>
