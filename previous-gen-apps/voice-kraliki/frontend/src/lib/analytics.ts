...
const SITE_DOMAIN = 'voice.kraliki.com';
...
export function trackVoiceCall(action: string, duration?: number): void {
  trackEvent('voice_call', { action, ...(duration !== undefined && { duration }) });
}

export function trackOperator(action: string, operator?: string): void {
  trackEvent('operator', { action, ...(operator && { operator }) });
}
