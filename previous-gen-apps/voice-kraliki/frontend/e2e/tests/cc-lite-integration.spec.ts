...
import { CC_LITE_TEST_DATA } from '../fixtures/test-data.js';

/**
 * CC-Lite Integration Tests
 *
 * These tests verify complete user flows across multiple pages
 * to ensure the application works as an integrated system.
 */
test.describe('CC-Lite Integration Tests', () => {
	...
	test.describe('Campaign to Voice Call Flow', () => {
		test('should configure voice call with campaign data', async ({ authenticatedPage }) => {
			...
			await voiceCallsPage.setAIInstructions(CC_LITE_TEST_DATA.testAIInstructions);
			await voiceCallsPage.setCompanyDetails(
				0,
				CC_LITE_TEST_DATA.testCompany.name,
				CC_LITE_TEST_DATA.testCompany.phone
			);

			// Verify configuration is set
			const instructions = await voiceCallsPage.getAIInstructions();
			expect(instructions).toBe(CC_LITE_TEST_DATA.testAIInstructions);

			const companyInput = authenticatedPage.locator('#company-0');
			await expect(companyInput).toHaveValue(CC_LITE_TEST_DATA.testCompany.name);
		});
	});
			...
			await authenticatedPage.route('**/api/v1/campaigns**', (route) => {
				...
				if (callCount === 1) {
					...
					route.fulfill({
						status: 200,
						contentType: 'application/json',
						body: JSON.stringify(CC_LITE_TEST_DATA.campaigns)
					});
				}
			});
