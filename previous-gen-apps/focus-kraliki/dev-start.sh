#!/usr/bin/env bash
#
# Focus by Kraliki - Development Starter Script
# Starts backend (port 8000) and frontend (port 5173) in development mode
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   Focus by Kraliki - Development Starter${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Clear ports
echo -e "${BLUE}[1/4] Clearing ports...${NC}"
kill_port $BACKEND_PORT
kill_port $FRONTEND_PORT
echo ""

# Check directories
echo -e "${BLUE}[2/4] Checking directories...${NC}"
...
echo -e "  Logs: ${YELLOW}$PROJECT_ROOT/backend.log${NC}"
echo ""
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        ...
        echo -e "${YELLOW}  Check logs: tail -f $PROJECT_ROOT/backend.log${NC}"
        exit 1
    fi
    sleep 1
done
...
echo -e "  Logs: ${YELLOW}$PROJECT_ROOT/frontend.log${NC}"
echo ""
...
for i in {1..60}; do
    ...
    if [ $i -eq 60 ]; then
        ...
        echo -e "${YELLOW}  Check logs: tail -f $PROJECT_ROOT/frontend.log${NC}"
        exit 1
    fi
    sleep 1
done
...
echo ""
echo -e "${BLUE}Logs:${NC}"
echo -e "  Backend:  ${YELLOW}tail -f $PROJECT_ROOT/backend.log${NC}"
echo -e "  Frontend: ${YELLOW}tail -f $PROJECT_ROOT/frontend.log${NC}"
echo ""
echo -e "${BLUE}Stop services:${NC}"
echo -e "  Kill all: ${YELLOW}kill $BACKEND_PID $FRONTEND_PID${NC}"
echo -e "  Or use:   ${YELLOW}./dev-stop.sh${NC}"
echo ""
