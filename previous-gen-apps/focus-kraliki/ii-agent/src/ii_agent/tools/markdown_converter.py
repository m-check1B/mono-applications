...
import traceback
import zipfile
from typing import Any, Union, TypedDict, IO
...
class ConversionOptions(TypedDict, total=False):
    """Options for document conversion methods.

    Attributes:
        file_extension: The file extension to use (e.g., '.pdf', '.docx')
        url: The original URL if converting from a web source
        mlm_client: Optional MLM client for image description
        mlm_model: Optional MLM model name
        mlm_prompt: Optional custom prompt for MLM image description
    """
    file_extension: str
    url: str
    mlm_client: Any
...
class YouTubeConverter(DocumentConverter):
    ...
    def _findKey(self, json: JSONValue, key: str) -> JSONValue:
        """Recursively search for a key in a JSON structure.

        Args:
            json: A JSON value (dict, list, or primitive type)
...
class MarkdownConverter:
    ...
    def convert(
        self, source: str | requests.Response, **kwargs: ConversionOptions
    ) -> DocumentConverterResult:
        """Convert a document from various sources to Markdown.

        Args:
            source: can be a string representing a path or url, or a requests.response object
            **kwargs: Conversion options (file_extension, url, mlm_client, etc.)

        Returns:
            DocumentConverterResult containing the converted content
        """
    ...
    def convert_local(
        self, path: str, **kwargs: ConversionOptions
    ) -> DocumentConverterResult:
    ...
    def convert_stream(
        self, stream: IO[bytes], **kwargs: ConversionOptions
    ) -> DocumentConverterResult:
        """Convert a stream/file-like object to Markdown.

        Args:
            stream: A file-like object that supports read() returning bytes
            **kwargs: Conversion options
    ...
    def convert_url(
        self, url: str, **kwargs: ConversionOptions
    ) -> DocumentConverterResult:
        """Convert a URL to Markdown by fetching and processing it.
    ...
    def convert_response(
        self, response: requests.Response, **kwargs: ConversionOptions
    ) -> DocumentConverterResult:
