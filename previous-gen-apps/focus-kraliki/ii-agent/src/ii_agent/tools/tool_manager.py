...
def get_system_tools(
    ...
    if tool_args:
        # Focus by Kraliki integration tools
        if tool_args.get("focus_tools", False):
            focus_api_base_url = tool_args.get(
                "focus_api_base_url",
                "http://127.0.0.1:3017"
            )
            agent_token = tool_args.get("agent_token")

            if agent_token:
                focus_tools = create_focus_tools(
                    focus_api_base_url=focus_api_base_url,
                    agent_token=agent_token
                )
                tools.extend(focus_tools)
                logger.info(f"Added {len(focus_tools)} Focus by Kraliki tools")
            else:
                logger.warning(
                    "Focus tools requested but agent_token not provided in tool_args"
                )
