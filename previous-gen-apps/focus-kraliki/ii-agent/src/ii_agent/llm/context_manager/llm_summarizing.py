...
class LLMSummarizingContextManager(ContextManager):
    ...
    def __init__(
        ...
        # Store the last summary to avoid fragile positional checks
...
"""
    ...
    def _apply_truncation_with_thinking_blocks(
        ...
        # Use stored summary as previous context
        previous_summary = self._last_summary if self._last_summary else "No events summarized"
        summary = self._generate_summary(events_to_summarize, previous_summary)

        # Store the summary for future truncations
    ...
    def _apply_truncation_without_thinking_blocks(
        ...
        # Use stored summary instead of fragile positional check
        summary_content = self._last_summary if self._last_summary else "No events summarized"
        summary_start_idx = self.keep_first

        # Check if there's an existing summary message at the expected position
        if (
            len(message_lists) > self.keep_first
            and message_lists[self.keep_first]
            and isinstance(message_lists[self.keep_first][0], TextResult)
            and message_lists[self.keep_first][0].text.startswith(
                "Conversation Summary:"
            )
        ):
            # Skip the existing summary message in the list
