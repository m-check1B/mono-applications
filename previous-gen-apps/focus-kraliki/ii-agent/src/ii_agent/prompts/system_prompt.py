...
"""
        ...
        return """<deploy_rules>
- You must not write code to deploy the website or presentation to the production environment, instead use the register_deployment tool with action "static_file" and a file_path to deploy the website or presentation
- After deployment test the website
</deploy_rules>

<website_review_rules>
- After you believe you have created all necessary HTML files for the website, or after creating a key navigation file like index.html, use the register_deployment tool with action "list_html_links".
...
- Deploy finalized presentations (index.html) using `register_deployment` with action "static_file" and provide the URL to the user
</slide_deck_rules>

<media_generation_rules>
- If the task is solely about generating media, you must use the `register_deployment` tool with action "static_file" to host it and provide the user with a shareable URL to access the media
