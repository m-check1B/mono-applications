...
from ii_agent.utils.constants import WORKSPACE_ALIAS
...
class PexpectSessionManager:
    ...
    def _format_output(
        ...
        if self.use_relative_path:
            return formated_output.replace(self.cwd, WORKSPACE_ALIAS).replace(
                self.work_dir, WORKSPACE_ALIAS
            )
        else:
        ...
        if new_directory:
            if self.use_relative_path:
                session.current_directory = new_directory.replace(
                    self.cwd, WORKSPACE_ALIAS
                ).replace(self.work_dir, WORKSPACE_ALIAS)
            else:
