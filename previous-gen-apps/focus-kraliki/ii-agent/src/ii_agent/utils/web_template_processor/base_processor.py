...
class BaseProcessor(ABC):
    ...
    def copy_project_template(self):
        """Copy the project template from the configured template directory.

        Uses the template_dir from SandboxSettings to locate templates.
        """
        template_source = f"{self.sandbox_settings.template_dir}/{self.template_name}"
        copy_result = self.terminal_client.shell_exec(
            self.sandbox_settings.system_shell,
            f"cp -rf {template_source} {self.project_name}",
