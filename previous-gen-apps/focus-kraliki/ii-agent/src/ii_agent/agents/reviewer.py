...
"""
    ...
    async def _generate_llm_response(
        self, messages: List[Any], tools: List[ToolCallParameters]
    ) -> Tuple[List[Any], Any]:
...
"""
        ...
        while remaining_turns > 0:
            ...
            max_context = getattr(
                self.context_manager, "max_context_length", float("inf")
            )
            ...
            model_response, _ = await self._generate_llm_response(
                truncated_messages_for_llm, all_tool_params
            )
            ...
            if len(pending_tool_calls) == 1:
                ...
                if tool_call.tool_name == "return_control_to_general_agent":
                    ...
                    current_messages = self.history.get_messages()
                    truncated_messages_for_llm = (
                        self.context_manager.apply_truncation_if_needed(
                            current_messages
                        )
                    )
                    self.history.set_message_list(truncated_messages_for_llm)
                    ...
                    if tool_output:
                        ...
                        self.logger_for_agent_logs.error(
                            "No text output in model response for review summary"
                        )
                        return ToolImplOutput(
    ...
    def get_tool_start_message(self, tool_input: dict[str, Any]) -> str:
        return "Reviewer started to analyze agent logs"
