...
const DEFAULT_SITE_DOMAIN = 'speak.verduona.dev';
...
function getSiteDomain(): string {
  if (!browser) return DEFAULT_SITE_DOMAIN;
...
export function trackSurvey(action: string, surveyId?: string): void {
  trackEvent('survey', { action, ...(surveyId && { survey_id: surveyId }) });
}

export function trackVoiceSession(action: string, duration?: number): void {
  trackEvent('voice_session', { action, ...(duration !== undefined && { duration }) });
}
